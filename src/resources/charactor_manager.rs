...
impl CharactorManager {
    ...
    pub fn create_charactor(& mut self, deploy: &Deploy, charactor_type: &CharactorType, charactor_subtype: &CharactorSubType, race_type: &RaceType) -> Charactor{
        ...
        let condition_max: HashMap<ConditionType, u16> = create_conditions_max(&race_config.conditions);
...
pub fn create_conditions(conditions: &HashMap<ConditionType, u16>) -> HashMap<ConditionType, u16> {
    let mut condition: HashMap<ConditionType, u16> = HashMap::from([
        (ConditionType::Pain, 0),
        (ConditionType::Fatigue, 0)
    ]);

    for (key, value) in conditions {
        match condition.get_mut(key) {
            Some(v) => {},
            None => println!("charactor_manager.create_conditions. There is no '{:?}' in condition", key),
        }
    }
...
pub fn create_conditions_max(conditions: &HashMap<ConditionType, u16>) -> HashMap<ConditionType, u16> {
    let mut condition: HashMap<ConditionType, u16> = HashMap::from([
        (ConditionType::Pain, 0),
        (ConditionType::Fatigue, 0)
    ]);
    ...
    for (key, value) in conditions {
        match condition.get_mut(key) {
            Some(v) => *v = *value,
            None => println!("charactor_manager.create_conditions_max. There is no '{:?}' in condition", key),
        }
    }
...
pub fn initialize_character_after_creation(charactor: &mut Charactor){
    ...
    for (key, _) in resists {
        match key {
            ...
            Resist::Knockdown => {
                let value_to_change: i16 = calculate_resist_knockdown_from_stat(*str, *end, *dex, *int);
                charactor::change_resist(
                    &mut charactor.resists,
                    &mut charactor.resists_cache,
                    &key, 
