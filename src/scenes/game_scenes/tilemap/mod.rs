...
impl Tilemap {
    ...
    fn generate_spot(&mut self, deploy: &Deploy, spot_setting: &SpotSetting) {
        ...
        for _ in 0..spot_setting.amount {
            ...
            let end_range_x: i32 = half_tilemap_width as i32 - max_width as i32 + 1;
            let starting_point_x = rng.gen_range(start_range_x..end_range_x);

            let start_range_y: i32 = -(half_tilemap_height as i32);
            let end_range_y: i32 = half_tilemap_height as i32 - max_height as i32 + 1;
    ...
    fn generate_river(&mut self, river_setting: &RiverSetting, deploy: &Deploy) {
        ...
        match river_type {
            RiverType::Horizontal => {
                let start_range_y: i32  = -(half_tilemap_height as i32) + current_width as i32 + offset as i32;
                let end_range_y: i32 = half_tilemap_height as i32 - current_width as i32 - offset as i32 + 1;
    ...
    fn generate_environment(&mut self, deploy: &Deploy, enviroument: u8) {
        ...
        let half_tilemap_height = (tilemap_height / 2) as u16;
        let half_tilemap_width = (tilemap_width / 2 ) as u16;
        let total_tiles: usize = self.total_tiles;
        let mut rng = rand::thread_rng();
        ...
        for i in 0..self.tilemap_tile_storage.len() {
            ...
            for i in 0..grid_multiplier {
                for j in 0..grid_multiplier {
                    let new_x = x as i32 - current_envirounment as i32 + j as i32 + half_tilemap_width as i32;
                    let new_y = y as i32 - current_envirounment as i32 + i as i32 + half_tilemap_height as i32;
                    if new_x < 0 
                        || new_y < 0 
                        || new_x >= tilemap_width as i32
                        || new_y >= tilemap_height as i32{
                            panic!( 
                                "tilemap.generate_environment. x: {}, y: {};", 
                                new_x, 
                                new_y
                            );
                    };
    ...
    fn find_cover_graphic_index_for_cover_tiles(&self, x: i32, y: i32) -> u8 {
        ...
        let top_index: i32 = (y + 1 + half_tilemap_height as i32) * tilemap_height as i32 + x + half_tilemap_width as i32;
        let left_top_index: i32 = (y + 1 + half_tilemap_height as i32) * tilemap_height as i32 + x - 1 + half_tilemap_width as i32;
        let left_index: i32 = (y + half_tilemap_height as i32) * tilemap_height as i32 + x - 1 + half_tilemap_width as i32;
        let right_top_index: i32 = (y + 1 + half_tilemap_height as i32) * tilemap_height as i32 + 1 + x + half_tilemap_width as i32;
        let right_index: i32 = (y + half_tilemap_height as i32) * tilemap_height as i32 + x + 1 + half_tilemap_width as i32;
        let bottom_index: i32 = (y - 1 + half_tilemap_height as i32) * tilemap_height as i32 + x + half_tilemap_width as i32;
        let left_bottom_index: i32 = (y - 1 + half_tilemap_height as i32) * tilemap_height as i32 + x - 1 + half_tilemap_width as i32;
        let right_bottom_index: i32 = (y - 1 + half_tilemap_height as i32) * tilemap_height as i32 + x + 1 + half_tilemap_width as i32;
        ...
        let left_top: bool = if left_top_index < 0 || left_top_index as usize >= total_tiles {
            false
        }else{
            match storage[left_top_index as usize].cover_type {
                CoverType::Shallow | CoverType::Water | CoverType::Ice => true,
                _ => false,
            }
        };

        let right_top: bool = if right_top_index < 0 || right_top_index as usize >= total_tiles {
            false
        }else{
            match storage[right_top_index as usize].cover_type {
                CoverType::Shallow | CoverType::Water | CoverType::Ice => true,
                _ => false,
            }
        };

        let left_bottom: bool = if left_bottom_index < 0 || left_bottom_index as usize >= total_tiles {
            false
        }else{
            match storage[left_bottom_index as usize].cover_type {
                CoverType::Shallow | CoverType::Water | CoverType::Ice => true,
                _ => false,
            }
        };
